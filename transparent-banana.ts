...
async function generateOnWhite(
  ...
  const response = await ai.models.generateContent({
    model,
    contents: `${prompt}. On a pure solid white #FFFFFF background`,
    config: {
...
async function extractObjectOnWhite(
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [
      {
        text: `Extract ${prompt} from this image and place it on a pure solid white #FFFFFF background. Keep the object exactly as it appears.`,
      },
      {
        inlineData: {
          mimeType: "image/png",
